# ── Tabela unificada de salário mínimo (1981–2025) ────────────────────────────

.sm_pnad <- data.table::data.table(
  ano = c(1981:2025),
  valor = as.double(c(
    # PNAD Anual (1981–2015)
    8464.80, 16608, 34776, 97176, 333120, 804, 2400, 18960, 249.48, 6056.31,
    42000, 522186.90, 9606, 70, 100, 112, 120, 130, 136, 151, 180, 200, 240,
    260, 300, 350, 380, 415, 465, 510, 545, 622, 678, 724, 788,
    # PNAD Contínua (2016–2025)
    880, 937, 954, 998, 1045, 1100, 1212, 1320, 1412, 1518
  ))
)


# ── Implementação interna — PNAD Anual ────────────────────────────────────────
...
.rowcalc_impl_A <- function(dt) {
  ...
  dt <- dt[(ano %between% c(1981,1990) & !is.na(resposta)) |
             (ano %between% c(1992,2015) & resposta == 1)]
  ...
  # PSU e STRATA ----
  .psu <- function(x) substr(as.character(x), 3, 6)
  .str <- function(x) substr(as.character(x), 1, 2)
  ...
  dt[, `:=` (
    psu    = paste0(ano, '_', uf_ref, '_', psu),
    strata = paste0(ano, '_', uf_ref, '_', strata)
  )]
  ...
  dt[, `:=` (
    ...
    raca = fcase(
      ano %in% c(1981,1983,1984,1985),           'Nd',
    ...
    area = fcase(local2 == 1, 'RM', default = 'UF'),
  ...
  dt[, `:=` (
    ...
    tem_filhos  = fifelse(any(cond_familia == 3, na.rm = TRUE), 'S', 'N'),
    agregados   = fifelse(any(cond_familia > 4,  na.rm = TRUE), 'S', 'N')
  ), by = domicilioid]
  ...
  ## SELECT FINAL =================================================================
  dt[, .(ano, uf, setor, area, domicilioid, psu, strata, peso, pessoas_dom, pessoas_fam,
         arranjo_familiar, agregados, cond_familia,
         sexo, raca, idade, escol, rpc,
         D1, D2, D3, B1, B2, B3, B4, V1, V2, E1, E2, E3, P1, P2)]
}


# ── Implementação interna — PNAD Contínua ─────────────────────────────────────
...
.rowcalc_impl_C <- function(dt) {
  ...
  dt[, `:=` (
    ...
    strata = paste0(ano, '_', uf_ref, '_', as.character(estrato))
  )]
  ...
  dt[, `:=` (
    ...
    raca        = fcase(
      v2010 == 1, 'Br', v2010 == 2, 'Pr', v2010 == 3, 'Pd',
      v2010 == 4, 'Am', v2010 == 5, 'In', default = 'Nd'
    ),
...
#' Prepara os indicadores intermediários para cálculo do MPI-LA a nível individual
#'
#' Detecta automaticamente o tipo de pesquisa a partir do ano presente nos dados.
#' Anos até 2015 → PNAD Anual; anos a partir de 2016 → PNAD Contínua.
#'
#' @param df data.frame ou data.table, criado por `process_pnad`, já filtrado
#' @param type Tipo de pesquisa: `'anual'` ou `'continua'`. Se `NULL` (padrão),
#'   detectado automaticamente a partir de `max(df$ano)`
#' @return data.table com indicadores intermediários do MPI-LA
#' @export

rowcalc_pnad <- function(df, type = NULL) {
